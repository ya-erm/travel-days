...
import { journalService, type JournalItem, type JournalOperation, type JournalSubscriber } from './journal';
import { userService } from './users';
import { useDB, type LocalDB } from './db';
import type { StoreNames, StoreValue } from 'idb';

type StorageName = Exclude<StoreNames<LocalDB>, 'users' | 'userSettings' | 'journal'>;

type EntityType = Omit<StoreValue<LocalDB, StorageName>, 'owner'>;

export class BaseService<T extends EntityType> implements JournalSubscriber {
  ...
  private async loadFromDB() {
    ...
    const allItems = (await db.getAllFromIndex(this._storageName, 'by-owner', user.uuid)) as unknown as T[];
    this._items.set({
  ...
  private async saveToDB(item: T) {
    const db = await useDB();
    const user = userService.getCurrentUser();
    await db.put(this._storageName, { ...item, owner: user.uuid });
  }
